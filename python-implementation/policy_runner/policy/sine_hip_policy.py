"""Sine-wave hip-pitch policy; all other joints held at latched start pose."""
...
LEFT_HIP_PITCH = int(JointIndex.LEFT_HIP_PITCH)
RIGHT_HIP_PITCH = int(JointIndex.RIGHT_HIP_PITCH)

# Oscillation about the latched hip pitch (absolute rad).
_HIP_AMP = 0.8
_PERIOD_S = 5.0
_FREQ_HZ = 1.0 / _PERIOD_S


class SineHipPolicy(Policy):
    """
    Hip pitches (10, 16) follow a sine wave about the start pose.
    ...
    def infer(self, obs: Observation) -> Action:
        ...
        delta = _HIP_AMP * math.sin(phase)
        left_q = float(hold[LEFT_HIP_PITCH]) + delta
        right_q = float(hold[RIGHT_HIP_PITCH]) + delta
